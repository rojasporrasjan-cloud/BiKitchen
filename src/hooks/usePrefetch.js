            ...
            if ('requestIdleCallback' in window) {
                window.requestIdleCallback(() => {
                    POPULAR_ROUTES.forEach(route => {
                        route().catch((err) => {
                            console.warn('[Prefetch] Failed to prefetch route:', err?.message || err);
                        });
                    });
                });
            } else {
...
export function usePrefetchOnHover() {
    ...
    const prefetch = useCallback((routeLoader) => {
        ...
        routeLoader().catch((err) => {
            console.warn('[Prefetch] Failed to prefetch on hover:', err?.message || err);
        });
    }, []);
        ...
        const timer = setTimeout(() => {
            const prefetch = async () => {
                try {
                    ...
                    console.warn('[PrefetchData] Failed to prefetch data:', err?.message || err);
                }
            };
...
export function PrefetchLink({ to, loader, children, className, ...props }) {
    const handleMouseEnter = () => {
        if (loader) {
            loader().catch((err) => {
                console.warn('[PrefetchLink] Failed to prefetch:', err?.message || err);
            });
        }
    };
