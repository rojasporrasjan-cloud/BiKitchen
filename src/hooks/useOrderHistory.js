...
export default function useOrderHistory() {
    ...
    // 2. Sincronización proactiva con Firestore — reactiva al login/logout
    useEffect(() => {
        let isMounted = true;
        let unsubByUid = null;
        ...
        const unsubAuth = onAuthStateChanged(auth, (user) => {
            ...
            if (unsubByUid) { unsubByUid(); unsubByUid = null; }
            if (unsubByEmail) { unsubByEmail(); unsubByEmail = null; }
            ...
            // Mapa para deduplicar resultados de diferentes queries
            ...
            const handleError = (err, queryType) => {
                if (err.code === 'permission-denied') {
                    console.warn(`[OrderHistory] Sin permisos para query ${queryType}`);
                } else {
                    console.error(`[OrderHistory] Error en query ${queryType}:`, err);
                }
                if (isMounted) setLoading(false);
            };
            ...
            unsubByUid = onSnapshot(qByUid,
                (snaps) => processSnapshot(snaps, 'UID'),
                (err) => handleError(err, 'UID')
            );

            // QUERY B: Por Correo (pedidos antiguos o invitados)
            const qByEmail = query(pedidosRef, where('correo', '==', userEmail));
            unsubByEmail = onSnapshot(qByEmail,
                (snaps) => processSnapshot(snaps, 'Email (Lower)'),
                (err) => handleError(err, 'Email')
            );
        });
