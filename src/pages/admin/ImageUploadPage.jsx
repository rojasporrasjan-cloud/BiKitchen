    ...
    return (
        <div className="relative" ref={dropdownRef}>
            ...
            <button
                ...
                className={`w-full text-xs px-2 py-1.5 border rounded-lg text-left flex items-center justify-between gap-1 ${
                            ...
                            filteredOptions.map(opt => (
                                <button
                                    ...
                                    className={`w-full text-left px-3 py-2 text-xs hover:bg-gray-100 ${
                                        value === opt.id ? 'bg-blue-50 text-blue-700' : ''
                                    }`}
                                >
                                    {opt.nombre}
                                </button>
...
export default function ImageUploadPage() {
    ...
    const convertHeicToJpg = async (file) => {
        try {
            ...
            // heic2any puede devolver un array o un blob
    ...
    // Subir todas las imágenes asignadas
    const uploadAllImages = async () => {
        ...
        console.log('[ImageUpload] Iniciando subida de', imagesToUpload.length, 'imágenes');
        ...
        for (const img of imagesToUpload) {
            try {
                console.log('[ImageUpload] Subiendo:', img.originalName, '→', img.assignedTo);
                
                const docRefImg = doc(db, 'imagenes', img.assignedTo);
                const prevSnap = await getDoc(docRefImg);
                const prevPath = prevSnap.exists() ? prevSnap.data()?.storagePath : null;
                const ts = Date.now();
                const storagePath = `imagenes/${img.categoria}/${img.assignedTo}_${ts}.webp`;
                console.log('[ImageUpload] Storage path:', storagePath);

                const storageRef = ref(storage, storagePath);
                const optimizedBlob = await optimizeToWebp(img.file, 1280);
                await uploadBytes(storageRef, optimizedBlob, { contentType: 'image/webp', cacheControl: 'public, max-age=31536000, immutable' });
                const url = await getDownloadURL(storageRef);
                console.log('[ImageUpload] URL obtenida:', url);
                ...
                console.log('[ImageUpload] ✓ Guardado en Firestore');
                ...
                if (error.code === 'storage/unauthorized') {
                    toast.error('Sin permisos para subir. Verifica las reglas de Storage.');
                } else if (error.code === 'permission-denied') {
                    toast.error('Sin permisos en Firestore. Verifica las reglas.');
                }
            }
        }
        ...
        if (errors.length > 0) {
            ...
            console.error('[ImageUpload] Errores:', errors);
        }
        
        if (uploaded > 0) {
            toast.success(`✅ ${uploaded} imágenes subidas correctamente`);
        }
    };
