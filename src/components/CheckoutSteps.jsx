...
export default function CheckoutSteps({ isOpen, onClose }) {
    ...
    const generateOrderSummary = () => {
        ...
        const currentTotal = orderComplete && lastOrderDetails ? lastOrderDetails.total : (isZoneOutOfCoverage() ? getTotalPrice() : getTotalWithShipping());
    ...
    useEffect(() => {
        localStorage.setItem('bikitchen-checkout-form', JSON.stringify(formData));
    }, [formData]);
    ...
    const createOrderWithPayment = async (orderNum, paypalDetails) => {
        ...
        const orderData = {
            ...
            envio_por_confirmar: isZoneOutOfCoverage(),
            plan: inferredPlan,
                                        ...
                                        >
                                                ...
                                                <div className="border-t border-gray-200 pt-2 space-y-1">
                                                    ...
                                                    {selectedZone && getSelectedZoneInfo() && (
                                                        <div className="flex justify-between text-sm">
                                                            <span className="text-gray-600 flex items-center gap-1">
                                                                <Truck size={12} />
                                                                Envío ({getSelectedZoneInfo().name.split('/')[0].trim()})
                                                            </span>
                                                            {isZoneOutOfCoverage() ? (
                                                                <span className="text-yellow-600 font-medium">Por confirmar</span>
                                                            ) : (
                                                    ...
                                                    <div className="flex justify-between font-bold text-lg pt-1 border-t border-gray-200 mt-2">
                                                        ...
                                                        <span className="text-bikitchen-orange">
                                                            {isZoneOutOfCoverage()
                                                                ? `₡${getTotalPrice().toLocaleString('es-CR')} + envío`
                                                                : `₡${getTotalWithShipping().toLocaleString('es-CR')}`
                                                            }
                                                        </span>
                                                    </div>
                                                </div>
                                            ...
                                            <div className={`rounded-xl p-4 ${formData.metodoPago === 'paypal'
                                                ...
                                                {formData.metodoPago === 'paypal' && (
                                                    <div className="mt-4">
                                                        <PayPalButton
                                                            totalCRC={isZoneOutOfCoverage() ? getTotalPrice() : getTotalWithShipping()}
                                                            orderData={{
