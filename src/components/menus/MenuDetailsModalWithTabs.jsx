...
import { Utensils, ShoppingCart, Plus, Minus, X, Coffee, Sun, Moon } from 'lucide-react';
...
export default function MenuDetailsModalWithTabs({ menuKey, isOpen, onClose, packInfo, mealTypes = ['almuerzo'], customTabContent = {} }) {
  ...
  const getDynamicPrice = () => {
    ...
    if (packInfo.isPromocionPack) {
      ...
      // Aplicar descuento de Firebase solo si el plan está en planesAplicables
  ...
  // Precio original antes del descuento Firebase (para mostrar tachado)
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => { if (!open) onClose(); }}>
      <DialogContent className="w-full h-full max-w-none sm:max-w-2xl p-0 overflow-hidden border-none bg-white rounded-none sm:rounded-[2.5rem] shadow-2xl overflow-y-auto sm:max-h-[90vh] custom-scrollbar">
        {/* HERO SECTION */}
        ...
        <div className="p-6 sm:p-10 space-y-6 bg-white text-slate-900">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              <span className="bg-orange-600 text-white text-[10px] font-black px-3 py-1.5 rounded-xl uppercase tracking-wider shadow-lg shadow-orange-500/20">Premium Pack</span>
              ...
              {mealTypes.map(type => {
                ...
                return (
                  <button
                    ...
                    className={`flex-1 flex items-center justify-center gap-2 py-2 px-3 rounded-xl font-black text-[10px] uppercase tracking-wider transition-all ${currentMealType === type ? 'bg-white text-slate-900 shadow-lg' : 'text-slate-400 hover:text-slate-600'
                      }`}
                  >
                    <Icon size={14} />
                    {meal.label}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div className="space-y-3">
              <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                <Utensils size={12} className="text-orange-500" /> Platos de la semana
              </h4>
                ...
                {hasCustomContent ? (
                  ...
                  <>
                    {currentDishes.map((dish, i) => (
                      <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }} className="flex items-center gap-3 bg-slate-50 border border-slate-100 p-2 rounded-xl hover:bg-slate-100 transition-colors">
                    ...
                    {!loading && currentDishes.length === 0 && !hasCustomContent && <p className="text-[11px] text-slate-300 italic py-4">Menú pendiente de actualizar...</p>}
                  </>
                )}
              </div>
            </div>
            ...
            <div className="space-y-5">
              <div className="space-y-2">
                <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Plan Actual</h4>
                <div className="grid grid-cols-3 gap-1 bg-slate-100 p-1 rounded-2xl border border-slate-100">
                  {['weekly', 'biweekly', 'monthly'].map(plan => (
                    <button
                      ...
                      className={`py-2 px-1 rounded-xl text-[10px] font-black transition-all ${selectedPlan === plan ? 'bg-orange-600 text-white shadow-lg' : 'text-slate-400 hover:text-slate-600'}`}
                    >
              ...
              <div className={`bg-gradient-to-br ${portionInfo.color} p-4 rounded-3xl flex justify-around items-center shadow-xl border border-white/20 text-white`}>
                <div className="text-center group"><span className="block text-xl mb-1">🍗</span><span className="text-[10px] font-black uppercase tracking-tighter">{portionInfo.protein}</span></div>
                <div className="text-center group"><span className="block text-xl mb-1">🥦</span><span className="text-[10px] font-black uppercase tracking-tighter">{portionInfo.veggies} Veg.</span></div>
                {!isNoCarbsMenu && currentMealType !== 'desayuno' && <div className="text-center group"><span className="block text-xl mb-1">🍚</span><span className="text-[10px] font-black uppercase tracking-tighter">{portionInfo.carbos} Carb.</span></div>}
                {currentMealType === 'desayuno' && <div className="text-center group"><span className="block text-xl mb-1">☕</span><span className="text-[10px] font-black uppercase tracking-tighter">Bebida</span></div>}
              </div>
            </div>
          </div>
          ...
          <div className="mt-auto pt-6 border-t border-slate-100 space-y-4">
            ...
            {/* Substitution picker — per-dish mode, uses current tab's dishes */}
            <SubstitutionPicker
              value={substitutions}
            ...
            {/* Price breakdown — original tachado + precio con descuento */}
            {originalTotalPrice && (
              <div className="bg-orange-50 border border-orange-100 rounded-2xl px-4 py-3 space-y-2">
                <div className="flex items-center justify-between">
                  <div className="flex flex-col">
                    <span className="text-[10px] font-black text-orange-500 uppercase tracking-widest">Con descuento</span>
                    <span className="text-sm font-bold text-slate-400 line-through">₡{originalTotalPrice.toLocaleString()}</span>
                  </div>
