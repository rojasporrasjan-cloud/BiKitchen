    ...
    const handleSubmitCard = async (e) => {
        ...
        try {
            ...
            const result = await processTransaction({
                ...
                cvv: cardData.cvv,
                // requestId fijo por orden (generado en CheckoutSteps) — retries usan el mismo ID
            ...
            // Notificar al parent (CheckoutSteps) sobre el error de pago
            if (onPaymentError) {
                onPaymentError({
                    orderId: orderId,
                    errorMessage: errorMessage || 'Error desconocido en el pago'
                });
            }
        } finally {
            setLoading(false);
            ...
            {isOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    <motion.div
                        ...
                        onClick={step !== 'processing' && step !== '3ds' ? onClose : undefined}
                        ...
                        <div className="flex-1 overflow-y-auto p-8 pt-2 custom-scrollbar">
                            {step === 'card' && (
                                <form onSubmit={handleSubmitCard} className="space-y-6">
                                    ...
                                    {/* Action Button - FIX #5: Deshabilitar durante procesamiento */}
                                    <button
                                        type="submit"
                                        disabled={loading || !gateway || step !== 'card'}
                                        className={`w-full py-5 rounded-[1.25rem] font-black text-lg shadow-xl transition-all flex items-center justify-center gap-3 ${
                                            loading || !gateway || step !== 'card'
                                                ? 'bg-gray-300 text-gray-500 cursor-not-allowed opacity-60'
                                                : 'bg-gradient-to-r from-orange-600 to-orange-700 text-white shadow-orange-200 hover:scale-[1.02] active:scale-[0.98]'
                                        }`}
                                    >
                                        {step !== 'card' ? (
                                            <>
                            ...
                            {step === '3ds' && (
                                <div className="flex flex-col items-center justify-center py-4 sm:py-8">
                                    ...
                                    <div
                                        ...
                                        className="w-full border-t border-b sm:border-2 sm:rounded-2xl border-gray-100 bg-white flex items-center justify-center relative"
                                        style={{ height: 'calc(100vh - 280px)', minHeight: '320px', overflowY: 'auto', WebkitOverflowScrolling: 'touch' }}
                                    >
                                        ...
                                        <button
                                            onClick={async () => {
                                                ...
                                                try {
                                                    ...
                                                    setError('Error al reiniciar el sistema de pago. Recarga la página.');
                                                }
                                            }}
