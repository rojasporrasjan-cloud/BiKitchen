...
export default function Navbar({ forceSolid = false }) {
    ...
    return (
        <div style={{ pointerEvents: 'none' }}>
            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[110] md:hidden">
                        <div className="absolute inset-0 bg-gradient-to-br from-orange-600 to-orange-400"></div>
                        <button onClick={() => setIsOpen(false)} className="absolute top-6 right-6 text-white p-3"><X size={32} /></button>
                        <div className="relative h-full flex flex-col justify-center items-center px-8 gap-6">
                            <img src="/assets/logo.png" className="h-16 w-auto bg-white p-4 rounded-3xl" alt="Logo" />
                            <div className="flex flex-col gap-4 w-full max-w-xs">
                                {navLinks.map((item) => (
                                    <Link key={item.name} to={item.path} onClick={() => setIsOpen(false)} className="bg-white/20 text-white font-bold py-4 px-6 rounded-2xl flex items-center justify-center gap-3">
                                        <item.icon size={22} />
                                        {item.name}
                                    </Link>
                                ))}
